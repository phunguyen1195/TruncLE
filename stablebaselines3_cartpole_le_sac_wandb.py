...
class WandbCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        self.angle.append(self.locals['infos'][0]['angle'])
        self.velocity.append(self.locals['infos'][0]['velocity'])
        # print ("here")
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            self.logger.record("mean angle of angle", np.mean(np.abs(self.angle)))
            self.logger.record("mean angle of velocity", np.mean(np.abs(self.velocity)))
            self.logger.dump(self.num_timesteps)
...
# env = gym.make("acrobot_le-v0", alpha = config['alpha'])
env = gym.make("cartpole_le-v0", alpha = config["alpha"], reward_type=config['reward_type'], max = config['max'])
...
model.learn(total_timesteps=config["total_timesteps"], callback=WandbCallback(model_save_path=f"models_for_paper/cartpole/{run.id}",eval_freq=200,verbose=1,),)

# model.save("models_for_paper/double_pendulum/sac_double_pendulum_tuned_062024")
