...
env = gym.make('pendulum_le-v0', alpha = config["alpha"], reward_type=config['reward_type'], max = config['max'])

model = SAC(config["policy_type"], env ,gamma=config['gamma'], 
    learning_rate=config['learning_rate'],
    tensorboard_log="pendulum_tensorboard/")
model.learn(total_timesteps=config["total_timesteps"], callback=WandbCallback(model_save_path=f"models_for_paper/Pendulum/{run.id}",eval_freq=200,verbose=1,),)
