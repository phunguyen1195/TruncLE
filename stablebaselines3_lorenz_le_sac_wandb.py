...
class WandbCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        self.meanx.append(self.locals['infos'][0]['mean x'])
        self.meany.append(self.locals['infos'][0]['mean y'])
        self.meanz.append(self.locals['infos'][0]['mean z'])
        self.act1.append(self.locals['infos'][0]['action'][0])
        # self.act2.append(self.locals['infos'][0]['action'][1])
        # self.act3.append(self.locals['infos'][0]['action'][2])
        # print ("here")
        ...
        if self.eval_freq > 0 and self.n_calls % self.eval_freq == 0:
            self.logger.record("mean x", np.mean(np.abs(self.meanx)))
            self.logger.record("mean y", np.mean(np.abs(self.meany)))
            self.logger.record("mean z", np.mean(np.abs(self.meanz)))
            self.logger.record("mean action x", np.mean(np.abs(self.act1)))
            # self.logger.record("mean action y", np.mean(np.abs(self.act2)))
            # self.logger.record("mean action z", np.mean(np.abs(self.act3)))
            # print ('here')
            self.logger.dump(self.num_timesteps)
...
env = gym.make('lorenz_le-v0', alpha = config["alpha"], reward_type=config['reward_type'], max = config['max'])

model = SAC(config["policy_type"], env ,gamma=config['gamma'], 
    learning_rate=config['learning_rate'],
    tensorboard_log="lorenz_tensorboard/")
model.learn(total_timesteps=config["total_timesteps"], callback=WandbCallback(model_save_path=f"models_for_paper/lorenz/{run.id}",eval_freq=200,verbose=1,),)
